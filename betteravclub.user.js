// ==UserScript==
// @name          Kinja Abatement
// @version       2.0
// @namespace     miken32
// @downloadURL   https://github.com/miken32/betteravclub/raw/master/betteravclub.user.js
// @icon          data:image/png;base64,
// @description   Tries to make The AV Club tolerable
// @author        Michael Newton
// @homepageURL   https://the-avocado.org/
// @match         https://avclub.com/*
// @match         https://*.avclub.com/*
// @match         https://*.kinja.com/*
// @match         https://*.gizmodo.com/*
// @match         https://*.jalopnik.com/*
// @match         https://*.io9.com/*
// @match         https://*.kotaku.com/*
// @match         https://*.lifehacker.com/*
// @match         https://*.splinternews.com/*
// @run-at        document-start
// @grant         none
// @require       https://ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js
// ==/UserScript==

console.log("foobar");
$(document).ready(function() {
  if (!$("div.post-content").length) {
    //
    //
    // front page
    //
    //

    // hide deals
    $("article[data-commerce-source=streamshare]").remove();

    // hide "you may also like" sidebar
    var sidebar = $("div#sidebar_wrapper").closest("aside");
    var content = sidebar.siblings("main");
    sidebar.remove();

    // expand the content to fill the page
    content.css("width", "100%");

    // get rid of network sites bar
    $("div.js_topbar").remove();

    // get rid of featured videos
    $("div.instream-native-video--frontpage").parent().remove();

    // shrink article images, place beside text
    var article_pic = $("article.js_post_item figure").parent();
    article_pic.each(function() {
      if ($(this).children("figure").css("max-width") === "none") {
        $(this).css("width", "30%").css("position", "absolute").css("left", 0);
        $(this).parent().css("margin-left", "35%");
      }
    });

    // do the same for videos
    var article_thumb = $("article.js_post_item iframe").parent().parent();
    article_thumb.css("width", "30%").css("position", "absolute").css("left", 0);
    article_thumb.parent().css("margin-left", "35%");

    // hide Kinja roundup
    $("a[href='https://kinja.com/kinjaroundup']").closest("article").remove();
  } else {

    //
    //
    // story page
    //
    //

    // hide "you may also like" sidebar
    $("section.sidebar").remove();

    // expand the content to fill more of the page
    $("section.main").css("width", "100%");
    $("div.main__content").css("max-width", "inherit");
    $("div.post-content > *, article.post .align--bleed, div.post__misc > div").css("max-width", "85%");
    $("section#js_discussion-region").css("margin", "0 auto");
    
    // allow comments to take more space (these are dynamic elements and aren't here when jquery loads)
    $("<style>").html(
      "div.discussion-header, article.reply, div.reply__content, div.reply__content p {max-width: none;}" + 
      "div.js_replies-wrapper {max-width: 85%; margin: 0 auto;}"
    ).appendTo("head");

    // get rid of network sites bar
    $("div.topnav").remove();

    // get rid of autoplay videos in article bodies
    var videoObs2 = new MutationObserver(function() {
      $("div.recent-video").closest("div.instream-native-video").remove();
    });
    videoObs2.observe(document.querySelector("div.post-content"), {
      subtree: true,
      childList: true
    });
    $("div.post-content div.instream-native-video").remove();

    // remove the infinite scroll items before they appear
    $("div.js_reading-list").remove();

    
  }
});
